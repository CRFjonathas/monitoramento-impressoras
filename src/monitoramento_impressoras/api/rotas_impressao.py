...
from src.monitoramento_impressoras.services.impressao_service import MotorImpressaoService
...
def disparar_impressao(dados: DisparoImpressao, db: Session = Depends(get_db)):
    ...
    impressora = db.query(Impressora).filter(Impressora.ip == dados.ip_impressora).first()
    
    if not impressora:
        raise HTTPException(status_code=404, detail=f"Impressora com IP {dados.ip_impressora} não encontrada no banco.")
