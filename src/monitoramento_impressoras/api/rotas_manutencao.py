...
from src.monitoramento_impressoras.schemas.incidente_schema import IncidenteCreate, IncidenteUpdate, IncidenteResponse

router = APIRouter()

@router.post("/", response_model=IncidenteResponse, status_code=status.HTTP_201_CREATED, summary="Registra um novo incidente")
def criar_incidente(incidente: IncidenteCreate, db: Session = Depends(get_db)):
    ...
    impressora = db.query(Impressora).filter(Impressora.id == incidente.impressora_id).first()
    if not impressora:
        raise HTTPException(status_code=404, detail="Impressora não encontrada no inventário.")
...
@router.get("/", response_model=List[IncidenteResponse], summary="Lista todos os incidentes")
def listar_incidentes(db: Session = Depends(get_db)):
    return db.query(Incidente).all()

@router.put("/{incidente_id}", response_model=IncidenteResponse, summary="Atualiza o status de um incidente")
def atualizar_incidente(incidente_id: int, incidente_atualizado: IncidenteUpdate, db: Session = Depends(get_db)):
...
@router.delete("/{incidente_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Remove um incidente")
