...
def test_obter_telemetria_com_sucesso(client):
    ...
    dados_mockados = {
        ...
        "itens": [{"name": "Toner Preto", "lastvalue": "80", "units": "%"}],
    }

    # O 'patch' intercepta a função real exatamente onde ela foi importada no arquivo de rotas
    with patch(
        "src.monitoramento_impressoras.api.rotas_telemetria.zabbix_service.buscar_dados_impressora"
    ) as mock_zabbix:
...
def test_obter_telemetria_impressora_nao_cadastrada(client):
    ...
    assert (
        ...
        == "Impressora com IP 10.0.0.99 não está no nosso inventário."
    )
